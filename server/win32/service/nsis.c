...
static void libacqd_usage(void);
static void libacqd_usage(void)
{
  printf("Usage: libacqd-nsis.exe [OPTION]\n");
...
static void create_kernel_service(void)
{
  ...
  acqd_create_service(LIBUSB_DRIVER_NAME_NT, display_name,
		     LIBUSB_DRIVER_PATH,
...
static void create_system_service(void)
{
  ...
  acqd_create_service(LIBUSB_SERVICE_NAME, display_name,
		     LIBUSB_SERVICE_PATH, 
...
static void delete_system_service(void)
{
  acqd_delete_service(LIBUSB_SERVICE_NAME);
}


static void start_system_service(void)
{
  acqd_start_service(LIBUSB_SERVICE_NAME);
}
...
static void stop_system_service(void)
{
  ...
  if(acqd_registry_is_nt())
    {
      acqd_stop_service(LIBUSB_SERVICE_NAME); 
    }
  else
    {
      do
...
static void restart_host_controllers(void)
{
  ...
  if(dev_info == INVALID_HANDLE_VALUE)
    {
      acqd_debug_error("restart_host_controllers(): getting device info "
		      "failed");
      return;
    }
  ...
  while(SetupDiEnumDeviceInfo(dev_info, dev_index, &dev_info_data))
    {
      if(acqd_registry_get_property(SPDRP_CLASS, dev_info, 
				   &dev_info_data, class, sizeof(class)))
	{
	  if(strstr(class, "acqd"))
	    {
	      if(!acqd_registry_set_device_state(DICS_PROPCHANGE, dev_info, 
						&dev_info_data))
		{
		  acqd_debug_error("restart_host_controllers: setting "
				  "device state failed");
		}
	    }
	}
...
static void install_composite_filter(void)
{
  acqd_registry_install_composite_filter();
}
