...
bool_t acqd_registry_is_nt(void)
{
  return GetVersion() < 0x80000000 ? 1 : 0;
}

bool_t acqd_registry_get_property(DWORD which, HDEVINFO dev_info, 
				 ...
				 char *buf, int size)
{
  ...
  if(acqd_registry_is_nt())
	...
	{
	  acqd_debug_error("acqd_reg_get_reg_property(): reading "
			  "registry key failed");
	  return FALSE;
	}
...
bool_t acqd_registry_set_property(DWORD which, HDEVINFO dev_info, 
				 SP_DEVINFO_DATA *dev_info_data, 
				 char *buf)
{
  char *val_name = NULL;
  ...
  switch(which)
    {
    case SPDRP_LOWERFILTERS:
      reg_type = acqd_registry_is_nt() ? REG_MULTI_SZ : REG_SZ;
      val_name = "LowerFilters";
      break;
    default:
      return 0;
  ...
  if(acqd_registry_is_nt())
    {
      if(!SetupDiSetDeviceRegistryProperty(dev_info, dev_info_data,
					   which, (BYTE *)tmp, size))
	{
	  acqd_debug_error("acqd_reg_set_reg_property(): setting "
...
bool_t acqd_registry_insert_filter(HDEVINFO dev_info,
				  SP_DEVINFO_DATA *dev_info_data, 
				  const char *filter_name)
{
  ...
  acqd_registry_get_property(SPDRP_LOWERFILTERS, dev_info, dev_info_data, 
  ...
  if(!acqd_registry_set_property(SPDRP_LOWERFILTERS, dev_info, 
...
bool_t acqd_registry_remove_filter(HDEVINFO dev_info, 
				  SP_DEVINFO_DATA *dev_info_data, 
				  const char *filter_name)
{
  char *p = NULL;
  ...
  if(!acqd_registry_get_property(SPDRP_LOWERFILTERS, dev_info, 
  ...
  return acqd_registry_set_property(SPDRP_LOWERFILTERS, dev_info, 
				   dev_info_data, filters);
}

bool_t acqd_registry_set_device_state(DWORD state, HDEVINFO dev_info, 
				     SP_DEVINFO_DATA *dev_info_data)
{
  ...
  if(!SetupDiSetClassInstallParams(dev_info, dev_info_data,
    ...
    {
      acqd_debug_error("acqd_registry_set_device_state(): setting class "
  ...
  if(!SetupDiCallClassInstaller(DIF_PROPERTYCHANGE, dev_info, 
				dev_info_data))
    {
      acqd_debug_error("acqd_registry_set_device_state(): calling class "
		      "installer failed");
      return FALSE;
    }
...
bool_t acqd_registry_is_root_hub(HDEVINFO dev_info, 
				SP_DEVINFO_DATA *dev_info_data)
{
  char hardware_id[REGISTRY_BUF_SIZE];
  
  if(!acqd_registry_get_property(SPDRP_HARDWAREID, dev_info, dev_info_data,
				hardware_id, sizeof(hardware_id)))
    {
      acqd_debug_error("acqd_registry_is_root_hub(): getting hardware id "
		      "failed");
      return FALSE;
    }
...
bool_t acqd_registry_is_composite_interface(HDEVINFO dev_info, 
					   SP_DEVINFO_DATA *dev_info_data)
{
  char hardware_id[REGISTRY_BUF_SIZE];
  ...
  if(!acqd_registry_get_property(SPDRP_HARDWAREID, dev_info, dev_info_data,
				hardware_id, sizeof(hardware_id)))
    {
      acqd_debug_error("acqd_registry_is_composite_interface(): getting "
		      "hardware id failed");
      return FALSE;
    }
...
bool_t acqd_registry_is_service_libacqd(HDEVINFO dev_info, 
				      SP_DEVINFO_DATA *dev_info_data)
{
  char service_name[REGISTRY_BUF_SIZE];

  if(!acqd_registry_get_property(SPDRP_SERVICE, dev_info, dev_info_data,
...
bool_t acqd_registry_is_composite_libacqd(HDEVINFO dev_info, 
					SP_DEVINFO_DATA *dev_info_data)
{
  HDEVINFO _dev_info;
  ...
  if(!acqd_registry_get_property(SPDRP_HARDWAREID, dev_info, dev_info_data,
				tmp, sizeof(tmp)))
    {
      acqd_debug_error("acqd_registry_is_composite_libacqd(): getting "
		      "hardware id failed");
      return FALSE;
    }
  ...
  strncpy(parent, tmp, sizeof("acqd\\vid_xxxx&pid_yyyy") - 1);
  strcat(parent, "&mi_");
  ...
  while(SetupDiEnumDeviceInfo(_dev_info, dev_index, &_dev_info_data))
    {
      if(!acqd_registry_get_property(SPDRP_HARDWAREID, _dev_info, 
	...
	
	{
	 ...
	 && acqd_registry_is_service_libacqd(_dev_info, &_dev_info_data))
	{
	  return TRUE;
	}
      dev_index++;
...
bool_t acqd_registry_install_composite_filter(void)
{
  HDEVINFO dev_info;
  SP_DEVINFO_DATA dev_info_data;
  int dev_index = 0;
  ...
  if(dev_info == INVALID_HANDLE_VALUE)
    {
      acqd_debug_error("acqd_registry_install_composite_filter(): getting "
		      "device info set failed");
      return FALSE;
    }
